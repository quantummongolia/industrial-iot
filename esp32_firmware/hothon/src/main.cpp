 ...
 *    Slave 1 : SPM33 — Хотхон ХС   (P, E)  → em14
 *
 *  Firebase RTDB:
...
// Slave IDs
constexpr uint8_t EM14_SLAVE = 1;  // SPM33 — Хотхон ХС
...
float cachedEm14EnergyKWh = 0.0f; bool cachedEm14EnergyValid = false;
...
void setup() {
  ...
  delay(100);
  Spm33_readCtPrimary(modbus, cfg::EM14_SLAVE);
...
void loop() {
  ...
  if (!ctPrimaryKnown[cfg::EM14_SLAVE]) {
    Spm33_readCtPrimary(modbus, cfg::EM14_SLAVE);
    delay(cfg::FRAME_GAP_MS);
  }
  ...
  if (totalizerCycle) {
    // ── Totalizer cycle: EM14 энерги
    lastTotalizerReadTime = now;
    Serial.println("[Cycle] Totalizer read (1 min interval)");

    em14 = Spm33_readEnergy(modbus, cfg::EM14_SLAVE);
    if (em14.energyOk) { cachedEm14EnergyKWh = em14.energyKWh; cachedEm14EnergyValid = true; }

    Serial.printf("[Totalizer] EM14:%s\n",
                  em14.energyOk ? String(em14.energyKWh, 1).c_str() : "#");
  } else {
    // ── Flow cycle: EM14 power
    em14 = Spm33_readPower(modbus, cfg::EM14_SLAVE);

    Serial.printf("[Flow] EM14:%s\n",
                  em14.powerOk ? String(em14.powerKW, 2).c_str() : "#");
  }

  // ── Алдаа escalate ──────────────────────────────────────────────
  bool anyOk = em14.powerOk || em14.energyOk;
  if (anyOk) {
    consecutiveReadFails = 0;

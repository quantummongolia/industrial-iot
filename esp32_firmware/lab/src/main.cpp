/*
 * ============================================================
 *  ESP32-WROOM Lab → Firebase Realtime Database
 * ============================================================
 *
 *  Нэг RS485 шугам дээр Modbus RTU @ 9600bps :
 *    Slave 1 : SPM33 — Лаборатор ХС1   (P, E)  → em16
 *    Slave 2 : SPM33 — Лаборатор ХС2   (P, E)  → em17
 *
 *  Firebase RTDB:
 *    /energy_meters/em16/{power_kw,total_energy_kwh}
 *    /energy_meters/em17/{power_kw,total_energy_kwh}
 *
...
// Slave IDs
constexpr uint8_t EM16_SLAVE = 1;  // SPM33 — Лаборатор ХС1
constexpr uint8_t EM17_SLAVE = 2;  // SPM33 — Лаборатор ХС2
...
private:
  ...
  void send(const uint8_t *data, size_t len) {
    ...
    Serial2.flush();             // бүх байт физикээр гартал хүлээнэ (TX done)
...
float cachedEm16EnergyKWh = 0.0f; bool cachedEm16EnergyValid = false;
float cachedEm17EnergyKWh = 0.0f; bool cachedEm17EnergyValid = false;
...
void setup() {
  ...
  delay(100);
  const uint8_t spmSlaves[2] = {cfg::EM16_SLAVE, cfg::EM17_SLAVE};
...
void loop() {
  ...
  {
    static const uint8_t spmSlaves[2] = {cfg::EM16_SLAVE, cfg::EM17_SLAVE};
    for (uint8_t s : spmSlaves) {
      if (!ctPrimaryKnown[s]) {
        Spm33_readCtPrimary(modbus, s);
        delay(cfg::FRAME_GAP_MS);
  ...
  Spm33Reading em16, em17;

  if (totalizerCycle) {
    // ── Totalizer cycle: EM16 + EM17 энерги
    lastTotalizerReadTime = now;
    Serial.println("[Cycle] Totalizer read (1 min interval)");

    em16 = Spm33_readEnergy(modbus, cfg::EM16_SLAVE);
    if (em16.energyOk) { cachedEm16EnergyKWh = em16.energyKWh; cachedEm16EnergyValid = true; }
    delay(cfg::FRAME_GAP_MS);

    em17 = Spm33_readEnergy(modbus, cfg::EM17_SLAVE);
    if (em17.energyOk) { cachedEm17EnergyKWh = em17.energyKWh; cachedEm17EnergyValid = true; }
    delay(cfg::FRAME_GAP_MS);

    Serial.printf("[Totalizer] EM16:%s EM17:%s\n",
                  em16.energyOk ? String(em16.energyKWh, 1).c_str() : "#",
                  em17.energyOk ? String(em17.energyKWh, 1).c_str() : "#");
  } else {
    // ── Flow cycle: EM16 + EM17 power
    em16 = Spm33_readPower(modbus, cfg::EM16_SLAVE);
    delay(cfg::FRAME_GAP_MS);
    em17 = Spm33_readPower(modbus, cfg::EM17_SLAVE);
    delay(cfg::FRAME_GAP_MS);

    Serial.printf("[Flow] EM16:%s EM17:%s\n",
                  em16.powerOk ? String(em16.powerKW, 2).c_str() : "#",
                  em17.powerOk ? String(em17.powerKW, 2).c_str() : "#");
  }

  // ── Алдаа escalate ──────────────────────────────────────────────
  bool anyOk = em16.powerOk || em16.energyOk || em17.powerOk || em17.energyOk;
  if (anyOk) {
    consecutiveReadFails = 0;

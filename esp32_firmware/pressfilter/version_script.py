...
Format expected: pressfilter-v1.2.3 (annotated or lightweight)
...
def _resolve_version() -> str:
    try:
        v = _git("describe", "--tags", "--match", "pressfilter-v*", "--dirty=-dirty")
        if v.startswith("pressfilter-v"):
            v = v[len("pressfilter-v"):]
        return v
    except Exception:
        pass
    try:

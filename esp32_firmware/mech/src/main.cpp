/*
 * ============================================================
 *  ESP32-S3-Zero Mech → Firebase Realtime Database
 * ============================================================
 *
 *  Нэг RS485 шугам дээр Modbus RTU @ 9600bps :
 *    Slave 1 : SPM33 — Механик ХС   (P, E)  → em15
 *
 *  Firebase RTDB:
 *    /energy_meters/em15/{power_kw,total_energy_kwh}
 *
...
// Slave IDs
constexpr uint8_t EM15_SLAVE = 1;  // SPM33 — Механик ХС
...
// Индекс нь slave ID (хамгийн их slave = 1 тул [2] хангалттай).
static uint16_t ctPrimary[2] = {1, 1};
static bool ctPrimaryKnown[2] = {false, false};
...
float cachedEm15EnergyKWh = 0.0f; bool cachedEm15EnergyValid = false;
...
void loop() {
  ...
  if (!ctPrimaryKnown[cfg::EM15_SLAVE]) {
    Spm33_readCtPrimary(modbus, cfg::EM15_SLAVE);
    delay(cfg::FRAME_GAP_MS);
  }
  ...
  if (totalizerCycle) {
    // ── Totalizer cycle: EM15 энерги
    lastTotalizerReadTime = now;
    Serial.println("[Cycle] Totalizer read (1 min interval)");

    em15 = Spm33_readEnergy(modbus, cfg::EM15_SLAVE);
    if (em15.energyOk) { cachedEm15EnergyKWh = em15.energyKWh; cachedEm15EnergyValid = true; }

    Serial.printf("[Totalizer] EM15:%s\n",
                  em15.energyOk ? String(em15.energyKWh, 1).c_str() : "#");
  } else {
    // ── Flow cycle: EM15 power
    em15 = Spm33_readPower(modbus, cfg::EM15_SLAVE);

    Serial.printf("[Flow] EM15:%s\n",
                  em15.powerOk ? String(em15.powerKW, 2).c_str() : "#");
  }

  // ── Алдаа escalate ──────────────────────────────────────────────
  bool anyOk = em15.powerOk || em15.energyOk;
  if (anyOk) {
    consecutiveReadFails = 0;

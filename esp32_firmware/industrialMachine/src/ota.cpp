...
// OTA task ↔ main loop хооронд солилцох state.
struct OtaProgress {
...
String deriveId() {
  ...
  WiFi.macAddress(mac);
  char buf[32];
  snprintf(buf, sizeof(buf), "industrialMachine_%02x%02x%02x%02x%02x%02x",
...
void publishBootState(FirebaseData* fb) {
  ...
  FirebaseJson j;
  j.set("family", "industrialMachine");
...
void publishProgressIfChanged(FirebaseData* fb) {
  ...
  if (!progress.active && strlen(progress.stage) == 0) return;
...
void selfTestIfReady() {
  ...
  if (state == ESP_OTA_IMG_PENDING_VERIFY) {
    ...
    j.set("validated_at/.sv", "timestamp");

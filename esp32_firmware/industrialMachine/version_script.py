...
Format expected: industrialMachine-v1.2.3 (annotated or lightweight)
...
def _resolve_version() -> str:
    try:
        v = _git("describe", "--tags", "--match", "industrialMachine-v*", "--dirty=-dirty")
        if v.startswith("industrialMachine-v"):
            v = v[len("industrialMachine-v"):]
        return v
    except Exception:
        pass
    try:
